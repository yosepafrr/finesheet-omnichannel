...
class ShopeeService
{
    ...
    public function updateStock(Store $store, string $itemId, string $modelId, int $stock)
    {
        ...
        Log::info('Shopee - Update Stock', [
            ...
            'failure_list' => is_array($result)
                ? data_get($result, 'response.failure_list', [])
                : [],
        ]);
        ...
        if (! is_array($result)) {
            throw new RuntimeException('Shopee mengembalikan respons pembaruan stok yang tidak valid.');
        }

        $error = trim((string) ($result['error'] ?? ''));
        $failures = data_get($result, 'response.failure_list', []);
        $failures = is_array($failures) ? $failures : [];

        if ($error !== '' || $failures !== []) {
            $failureReasons = array_values(array_filter(array_map(
                fn ($failure) => is_array($failure)
                    ? ($failure['failed_reason'] ?? $failure['message'] ?? $failure['error'] ?? null)
                    : null,
                $failures
            )));
            $reason = implode('; ', $failureReasons)
                ?: (string) ($result['message'] ?? $error ?: 'Alasan tidak diberikan');

            throw new RuntimeException("Shopee menolak pembaruan stok: {$reason}");
        }
